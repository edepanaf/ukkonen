...
This file, written by Elie de Panafieu, is available on git hub.
...
The tree has additional edges, called the 'suffix edges'.
...
the longest suffix that has at least two occurrences in 'word',
...
If 'length == 0', then 'position' has no particular constraint.
...
>>> ukkonen('abcabxabcd')
(0, 3, 0, [{'a': (0, 2, 1), 'b': (1, 1, 2), 'c': (2, 1, 5), 'x': (5, inf, 'leaf'), 'd': (9, inf, 'leaf')}, {'x': (5, inf, 'leaf'), 'c': (2, 1, 3), 'suffix': 2}, {'x': (5, inf, 'leaf'), 'c': (2, 1, 4), 'suffix': 0}, {'d': (9, inf, 'leaf'), 'a': (3, inf, 'leaf'), 'suffix': 4}, {'d': (9, inf, 'leaf'), 'a': (3, inf, 'leaf'), 'suffix': 5}, {'d': (9, inf, 'leaf'), 'a': (3, inf, 'leaf'), 'suffix': 0}])

>>> ukkonen('abcacdae')
(0, 0, 0, [{'a': (0, 1, 1), 'b': (1, inf, 'leaf'), 'c': (2, 1, 2), 'd': (5, inf, 'leaf'), 'e': (7, inf, 'leaf')}, {'c': (4, inf, 'leaf'), 'b': (1, inf, 'leaf'), 'suffix': 0, 'e': (7, inf, 'leaf')}, {'d': (5, inf, 'leaf'), 'a': (3, inf, 'leaf'), 'suffix': 0}])

>>> ukkonen('aabaabbc')
(0, 2, 0, [{'a': (0, 1, 1), 'b': (2, 1, 4), 'c': (7, inf, 'leaf')}, {'b': (2, 1, 3), 'a': (1, 2, 2), 'suffix': 0}, {'b': (6, inf, 'leaf'), 'a': (3, inf, 'leaf'), 'suffix': 3}, {'b': (6, inf, 'leaf'), 'a': (3, inf, 'leaf'), 'suffix': 4}, {'b': (6, inf, 'leaf'), 'a': (3, inf, 'leaf'), 'suffix': 0, 'c': (7, inf, 'leaf')}])

>>> ukkonen('abcabdabc')
(1, 2, 1, [{'a': (0, 2, 1), 'b': (1, 1, 2), 'c': (2, inf, 'leaf'), 'd': (5, inf, 'leaf')}, {'d': (5, inf, 'leaf'), 'c': (2, inf, 'leaf'), 'suffix': 2}, {'d': (5, inf, 'leaf'), 'c': (2, inf, 'leaf'), 'suffix': 0}])

"""

import math
# with 20 lines of comments
def ukkonen(word):
  tree, node, position, length, length_node_child, child = [{}], 0, 0, 0, 0, 0
  ROOT, SUFFIX, LEAF = 0, 'suffix', 'leaf'
  for p in range(len(word)):
    letter = word[p]
    previous_node = ROOT
    # The next loop treats canonical implicit nodes which do not have a 'letter' continuation.
    while length > 0 and letter != word[position + length]:
      ...
      new_node = len(tree)
      tree.append({letter: (p, math.inf, LEAF),
    ...
    while node != ROOT and letter not in tree[node] and length == 0:
      ...
      tree[node][letter] = (p, math.inf, LEAF)
    ...
    if node == ROOT and letter not in tree[ROOT]:
      # addition of the leaf
      tree[ROOT][letter] = (p, math.inf, LEAF)
      # The new longest suffix with at least two occurrences is the empty word, so length = 0.
      length = 0
      # addition of the suffix link
      if previous_node != ROOT:
    ...
    # Finally, we consider the cases where we have reached an implicit or explicit node which has a 'letter' transition.
      ...
      # This 'else' deals with implicit nodes.
      else:
        position, length_node_child, child = tree[node][word[position]]
      # The addition of 'letter' increases the length, and the node becomes implicit and possibly non-canonical.
      length += 1
